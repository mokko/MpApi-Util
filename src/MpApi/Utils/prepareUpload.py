...
class PrepareUpload(BaseApp):
    ...
    def _init_sheet(self, workbook: Workbook) -> any:  # openpyxl.worksheet
        ...
        if ws.max_row > 2:  # already filled in?
            return ws
    ...
    def scan_disk(self):
        def _per_row(*, rc: int, path: Path) -> None:
            """
            rc: row count
            ...
            self.ws[f"A{rc}"] = filename
            self.ws[f"B{rc}"] = identNr
            self.ws[f"G{rc}"] = str(path)
