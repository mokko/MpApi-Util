...
#        else:
    ...
    def _update_schemas(self, *, data):
        if not hasattr(self, "schema_db"):
            self._load_schemas()
        itemL = data.xpath("/m:application/m:modules/m:module/m:moduleItem/m:repeatableGroup[@name = 'ObjObjectNumberGrp']/m:repeatableGroupItem")
...
#
    ...
    def new_from_node(self, *, node) -> IdentNr:
        iNr = IdentNr()
        try:
            iNr.text = node.xpath("m:dataField[@name = 'InventarNrSTxt']/m:value/text()", namespaces=NSMAP)[0]
        except:
            # without text, i cant make schema and without schema I cant save in json...
            raise ValueError ("No InventarNrSTxt found!" + etree.tostring(node, pretty_print=True, encoding="unicode"))
        try:
            iNr.part1 = node.xpath("m:dataField[@name = 'Part1Txt']/m:value/text()", namespaces=NSMAP)[0]
        except:
            iNr.part1 = None
        try:
            iNr.part2 = node.xpath("m:dataField[@name = 'Part2Txt']/m:value/text()", namespaces=NSMAP)[0]
        except:
            iNr.part2 = None
        try:
            iNr.part3 = node.xpath("m:dataField[@name = 'Part3Txt']/m:value/text()", namespaces=NSMAP)[0]
        except:
            iNr.part3 = None

        iNr.schemaId = int(node.xpath("m:moduleReference[@name = 'InvNumberSchemeRef']/m:moduleReferenceItem/@moduleItemId", namespaces=NSMAP)[0])
