...
def create_record(*, row: tuple, conf: dict, act: bool) -> None:
    ...
    print(f"DDD: {row[2].value}")
    set_ident(
        recordM, ident=row[2].value, institution=conf["institution"]
    )  # from Excel as str
    ...
    if missing_info:
        msg = f"Not creating record in RIA '{row[2].value}' since missing info"
        logging.error(msg)
        print(f">> {msg}")
        return missing_info
    if act:
        ...
        msg = f"Created record {objId} in RIA '{row[2].value}'"
        logging.error(msg)
        print(f">> {msg}")
    else:
...
def per_row(*, idx: int, row: Cell, conf: dict, act: bool) -> None:
    ident = row[2].value  # from Excel as str
